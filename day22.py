        ...
        while not f.find_first_missing(f) is None:
            ...
            while True:
                ...
                if ov.includes(f_m.coords[1]):
                    su = rows[j][0].find_col(ov.l, rows[j][0])
                    sl = f_m.find_col(ov.l, f_m)
...
def part2(file) -> int:
    def make_edge(rows, ps, px, py, cx, cy, sx, sy, dx, dy, dm):
        ...
        x = rows[x_coord[0]][0].find_col(x_coord[1], rows[x_coord[0]][0])
        y = rows[y_coord[0]][0].find_col(y_coord[1], rows[y_coord[0]][0])
    ...
    for i, (fu, iu) in enumerate(rows[:-1]):
        ...
        su = fu.find_col(ov.l, fu)
        sl = fl.find_col(ov.l, fl)
        su.zip_nodes(sl, ov.length())
